...
function itemEditarGrupo(id) {
  return `
    <a class="row-actions__item" href="novo-grupo-produto.html?id=${id}">
...
async function renderTabela() {
  ...
  const filtrados = filtro ? grupos.filter(g => g.nome.toLowerCase().includes(filtro)) : grupos

  document.getElementById('grupos-count-badge').textContent = `${filtrados.length} grupo${filtrados.length !== 1 ? 's' : ''}`

  if (filtrados.length === 0) {
    tbody.innerHTML = '<tr><td colspan="3" class="table-empty-cell">Nenhum grupo encontrado</td></tr>'
    return
  }
...
function setupAcoes() {
  document.getElementById('grupos-tbody').addEventListener('click', e => {
    ...
    fecharTodosOsMenus()
    if (!jaAberto) abrirMenuParaToggle(toggle, id, itemEditarGrupo(id))
  })
